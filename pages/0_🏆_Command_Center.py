...
st.set_page_config(page_title="H2 Sports — Command Center", page_icon="🏆", layout="wide")
...
def yesterday_catches(date_str):
    ...
    return R.homer_report(plays, results)["caught"], len(results)
...
with left:
    ...
    if plays:
        tdf = pd.DataFrame(plays[:8])[["Conviction", "Player", "Market", "Side", "Line", "ModelProb", "Why"]]
        st.dataframe(
...
st.divider()
st.subheader("🎯 The model caught these — last night's non-obvious bats")
st.caption("Hitters who homered and sat in the model's top HR plays before the game. Surfaced by "
           "matchup, platoon, Statcast, and weather — not name value. (Exploratory; see Retrospective.)")
try:
    caught, _ = yesterday_catches(yest)
except Exception:
    caught = []
if caught:
    ...
    st.dataframe(
        cdf[["Player", "HR", "ModelProb", "Pre-game rank"]].rename(columns={"ModelProb": "Model HR%"})
        .style.format({"Model HR%": "{:.0%}"}), hide_index=True, use_container_width=True)
else:
    st.caption("No model-ranked hitters homered last night, or results aren't final yet.")
